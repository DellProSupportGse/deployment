...
IF($CfgRdma -eq "True"){
  ...
  # RDMA QOS setting for Mellanox
      If((Get-NetAdapter $MgmtNics | Select InterfaceDescription) -imatch 'Mellanox|NVIDIA' -or ((Get-PhysicalDisk | Where-Object{$_.MediaType -imatch 'HDD'}).count -eq 0)){
          ...
          # Enable QoS for the network adapter ports.
              If((Get-NetAdapter $MgmtNics | Select InterfaceDescription) -imatch 'Mellanox|NVIDIA'){
